...
const ShopByBrandSection: React.FC = () => {
  ...
  useEffect(() => {
    const fetchProductsAndGroupByBrand = async () => {
      try {
        ...
        const brandsArray = Array.from(brandMap.entries()).map(([name, products]) => ({
          name,
  ...
  return (
    <section className="w-full mb-12">
      {/* Title Row */}
      <div className="flex bg-gradient-to-r from-blue-600 to-purple-700 rounded-t-md items-center justify-between mb-0 px-4 sm:px-6 lg:px-8 py-3 shadow-md">
      ...
      <div className="bg-white rounded-b-md shadow-lg p-6">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {brands.slice(0, 6).map((brand, index) => (
            <div
              key={brand.name}
              className="group bg-gradient-to-br from-gray-50 to-white rounded-xl border border-gray-200 hover:border-blue-300 hover:shadow-xl transition-all duration-300 overflow-hidden"
            >
              {/* Brand Header */}
              ...
              <div className="p-4">
                <div className="grid grid-cols-3 gap-2 mb-4">
                  {brand.products.map((product) => (
                    ...
                    >
                      <div className="aspect-square bg-gray-50 rounded-lg overflow-hidden border hover:border-blue-300 transition-all duration-200">
                        <img
                      ...
                      <p className="text-xs text-gray-600 mt-1 truncate group-hover/product:text-blue-600 transition-colors">
                        {product.title}
                      </p>
                    </Link>
                  ))}
                ...
                <Link
                  ...
                  className="block w-full text-center bg-gradient-to-r from-blue-600 to-purple-600 text-white py-2 rounded-lg font-medium hover:from-blue-700 hover:to-purple-700 transition-all duration-200 transform hover:scale-105"
                >
