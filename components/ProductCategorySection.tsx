...
const ProductCategorySection: React.FC<ProductCategorySectionProps> = ({
      ...
      try {
        ...
        const filtered = productsData.filter((product: any) => {
          ...
          if (product.category && product.category.slug === categorySlug)
            return true;
          // Check subcategory slug (if present)
          if (product.subcategory && product.subcategory.slug === categorySlug)
            return true;
          return false;
        });
  ...
  return (
    <section className="mb-12 mx-4 md:mx-6 lg:mx-8">
      <div className="flex bg-white items-center justify-between mb-8  px-6 py-2 shadow-lg rounded-lg">
        <h2 className="text-2xl md:text-2xl font-bold text-black">{title}</h2>
        <Link
          href={`/shop/${categorySlug}`}
          className="group flex items-center gap-2 text-black hover:text-gray-200 font-medium transition-colors duration-200"
        >
          <span>See More</span>
          <ChevronRightIcon
            size={16}
      ...
      <div className="relative group">
        ...
        <div
          className={`flex gap-4 md:gap-6 overflow-x-auto scrollbar-hide px-4 sm:px-6 lg:px-8 pb-4 scroll-smooth ${
            layout === "featured" ? "justify-center" : ""
          }`}
        >
          {products
            .slice(0, layout === "featured" ? 3 : 8)
            .map((product: Product) => {
                ...
                >
                  <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-6 hover:shadow-2xl hover:border-red-200 hover:shadow-red-100/50 transition-all duration-500 transform hover:-translate-y-2 hover:scale-[1.02] relative">
                    ...
                    <Link href={`/product/${product.slug}`}>
                      <div
                        className={`relative mb-4 aspect-square overflow-hidden rounded-xl bg-gradient-to-br from-gray-50 via-white to-gray-100 cursor-pointer ${
                          layout === "compact"
                            ? "h-40"
                            : layout === "featured"
                            ? "h-64"
