...
const WishItem = ({
  ...
  const deleteItemFromWishlist = async (productId: string) => {
    ...
    if (userId) {
      ...
      fetch(`${BACKEND_URL}/api/wishlist/${userId}/${productId}`, {method: "DELETE"}).then(
        (response) => {
  ...
  return (
    <tr className="hover:bg-gray-100 cursor-pointer">
      ...
      <td>
        <button className="btn btn-xs bg-red-500 text-white border border-red-500 hover:bg-white hover:text-red-500 text-sm">
          <FaHeartCrack />
          <span
